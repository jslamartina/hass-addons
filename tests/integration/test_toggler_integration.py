...
from cync_controller.harness.toggler import toggle_device_with_retry
from cync_controller.metrics import start_metrics_server
...
async def test_packet_format_validation(
    ...
    payload_bytes = raw[7:]
    payload_dict = cast(dict[str, object], json.loads(payload_bytes.decode("utf-8")))
...
async def test_metrics_endpoint_accessible(
    ...
    metrics_text = ""
    try:
        with cast(HTTPResponse, urllib.request.urlopen(metrics_url, timeout=5)) as http_response:  # noqa: S310
            metrics_text = http_response.read().decode("utf-8")
