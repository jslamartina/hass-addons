...
from cync_controller.protocol.exceptions import PacketDecodeError
...
def test_decode_packet_too_short() -> None:
    ...
    with pytest.raises(PacketDecodeError, match="too_short"):
        _ = CyncProtocol.decode_packet(bytes([0x23, 0x00]))
...
def test_encode_handshake_invalid_endpoint() -> None:
    ...
    with pytest.raises(ValueError, match="Endpoint must be 5 bytes"):
        _ = CyncProtocol.encode_handshake(bytes(3), auth_code)

    # Too long
    with pytest.raises(ValueError, match="Endpoint must be 5 bytes"):
        _ = CyncProtocol.encode_handshake(bytes(7), auth_code)
...
def test_encode_handshake_invalid_auth_code() -> None:
    ...
    with pytest.raises(ValueError, match="Auth code must be 16 bytes"):
        _ = CyncProtocol.encode_handshake(endpoint, bytes(10))

    # Too long
    with pytest.raises(ValueError, match="Auth code must be 16 bytes"):
        _ = CyncProtocol.encode_handshake(endpoint, bytes(20))
...
def test_encode_data_packet_invalid_endpoint() -> None:
    ...
    # Too short
    with pytest.raises(ValueError, match="Endpoint must be 5 bytes"):
        _ = CyncProtocol.encode_data_packet(bytes(3), msg_id, payload)

    # Too long
    with pytest.raises(ValueError, match="Endpoint must be 5 bytes"):
        _ = CyncProtocol.encode_data_packet(bytes(7), msg_id, payload)
...
def test_encode_data_packet_invalid_msg_id() -> None:
    ...
    with pytest.raises(ValueError, match="msg_id must be 2 bytes"):
        _ = CyncProtocol.encode_data_packet(endpoint, bytes(1), payload)

    # Too long
    with pytest.raises(ValueError, match="msg_id must be 2 bytes"):
        _ = CyncProtocol.encode_data_packet(endpoint, bytes(3), payload)
