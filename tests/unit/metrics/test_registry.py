...
class Collectable(Protocol):
    def collect(self) -> Iterable[MetricProtocol]: ...
...
def collect_samples(metric: Collectable) -> list[Sample]:
    """Collect and flatten samples from a Prometheus metric."""
    collected = list(metric.collect())
    if not collected:
        return []
    return list(collected[0].samples)
...
class TestACKMetrics:
    ...
    def test_record_ack_received(self) -> None:
        ...
        samples = collect_samples(registry.tcp_comm_ack_received_total)
    ...
    def test_record_ack_timeout(self) -> None:
        ...
        samples = collect_samples(registry.tcp_comm_ack_timeout_total)
    ...
    def test_record_idempotent_drop(self) -> None:
        ...
        samples = collect_samples(registry.tcp_comm_idempotent_drop_total)
        assert any(s.labels == {"device_id": "device1"} for s in samples)

    def test_record_retry_attempt(self) -> None:
        """Test record_retry_attempt helper."""
        registry.record_retry_attempt("device1", 1)
        samples = collect_samples(registry.tcp_comm_retry_attempts_total)
    ...
    def test_record_message_abandoned(self) -> None:
        ...
        samples = collect_samples(registry.tcp_comm_message_abandoned_total)
...
class TestConnectionMetrics:
    ...
    def test_record_connection_state(self) -> None:
        ...
        samples = collect_samples(registry.tcp_comm_connection_state)
    ...
    def test_record_handshake(self) -> None:
        ...
        samples = collect_samples(registry.tcp_comm_handshake_total)
    ...
    def test_record_reconnection(self) -> None:
        ...
        samples = collect_samples(registry.tcp_comm_reconnection_total)
    ...
    def test_record_heartbeat(self) -> None:
        ...
        samples = collect_samples(registry.tcp_comm_heartbeat_total)
        assert any(s.labels == {"device_id": "device1", "outcome": "success"} for s in samples)
...
class TestDedupCacheMetrics:
    ...
    def test_record_dedup_cache_size(self) -> None:
        ...
        samples = collect_samples(registry.tcp_comm_dedup_cache_size)
    ...
    def test_record_dedup_cache_hit(self) -> None:
        ...
        samples = collect_samples(registry.tcp_comm_dedup_cache_hits_total)
    ...
    def test_record_dedup_cache_eviction(self) -> None:
        ...
        samples = collect_samples(registry.tcp_comm_dedup_cache_evictions_total)
        assert len(samples) > 0
...
class TestPerformanceMetrics:
    ...
    def test_record_state_lock_hold(self) -> None:
        ...
        samples = collect_samples(registry.tcp_comm_state_lock_hold_seconds)
        assert len(samples) > 0
...
class TestDeviceOperationMetrics:
    ...
    def test_record_mesh_info_request(self) -> None:
        ...
        samples = collect_samples(registry.tcp_comm_mesh_info_request_total)
        assert any(s.labels == {"device_id": "device1", "outcome": "success"} for s in samples)
    ...
    def test_record_device_info_request(self) -> None:
        ...
        samples = collect_samples(registry.tcp_comm_device_info_request_total)
        assert any(s.labels == {"device_id": "device1", "outcome": "success"} for s in samples)
    ...
    def test_record_device_struct_parsed(self) -> None:
        ...
        samples = collect_samples(registry.tcp_comm_device_struct_parsed_total)
        assert any(s.labels == {"device_id": "device1"} for s in samples)
    ...
    def test_record_primary_device_violation(self) -> None:
        ...
        samples = collect_samples(registry.tcp_comm_primary_device_violations_total)
        assert len(samples) > 0
