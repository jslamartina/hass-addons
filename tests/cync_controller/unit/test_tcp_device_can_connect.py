...
@dataclass
class GlobalObjectStub:
    """Typed replacement for patched global module."""

    ncync_server: ServerStub
...
class TestCyncTCPDeviceCanConnect:
    ...
    async def test_can_connect_rejects_when_not_in_whitelist(
        ...
        with (
            ...
            global_stub = GlobalObjectStub(_create_server_mock())
            mock_get_g.return_value = global_stub

            # Try to connect from IP not in whitelist
            device = CyncTCPDevice(mock_reader, mock_writer, address="192.168.1.300")

            global_stub.ncync_server.tcp_conn_attempts["192.168.1.300"] = 1
    ...
    async def test_can_connect_accepts_when_in_whitelist(
        ...
        with (
            ...
            global_stub.ncync_server.tcp_conn_attempts["192.168.1.100"] = 0
