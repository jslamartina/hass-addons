...
class TestCyncTCPDevicePacketParsing0x83Advanced:
    ...
    @staticmethod
    def _configure_globals(mock_g: MagicMock) -> MagicMock:
        """Initialize patched global object with typed stubs."""
        server = MagicMock()
        server.configure_mock(
            devices={},
            remove_tcp_device=AsyncMock(return_value=None),
        )
    ...
    async def test_parse_0x83_firmware_device_version(self, real_tcp_device: CyncTCPDevice) -> None:
        ...
        with patch("cync_controller.devices.g") as mock_g:
            _ = self._configure_globals(mock_g)
    ...
    async def test_parse_0x83_internal_status_fa_db_13(self, real_tcp_device: CyncTCPDevice) -> None:
        ...
        with patch("cync_controller.devices.g") as mock_g:
            server = self._configure_globals(mock_g)
            server.devices = {0x34: CyncDevice(cync_id=0x34, name="Test Device")}
            server.parse_status = AsyncMock()
