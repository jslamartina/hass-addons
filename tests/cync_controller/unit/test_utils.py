...
signal_handler_fn: SignalHandlerFunc = cast(SignalHandlerFunc, signal_handler)  # type: ignore[reportUnknownMemberType]
...
async def run_async_cleanup() -> None:
    ...
    cleanup_func = cast(Callable[[], Awaitable[None]], _async_signal_cleanup)  # type: ignore[reportPrivateUsage]
    await cleanup_func()
