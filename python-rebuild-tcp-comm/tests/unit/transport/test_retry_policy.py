...
class TestTimeoutConfig:
    ...
    def test_default_timeout_config(self):
        ...
        assert config.ack_timeout_seconds == pytest.approx(0.1275)  # 51 * 2.5 / 1000
        assert config.handshake_timeout_seconds == pytest.approx(0.31875)  # 0.1275 * 2.5
        assert config.heartbeat_timeout_seconds == pytest.approx(10.0)  # max(0.1275 * 3, 10)
        assert config.cleanup_interval_seconds == pytest.approx(10.0)  # max(10, min(60, 0.1275/3))
    ...
    def test_custom_p99_timeout_config(self):
        ...
        # Verify calculated timeouts
        assert config.ack_timeout_seconds == pytest.approx(0.25)  # 100 * 2.5 / 1000
        assert config.handshake_timeout_seconds == pytest.approx(0.625)  # 0.25 * 2.5
        assert config.heartbeat_timeout_seconds == pytest.approx(10.0)  # max(0.25 * 3, 10)
        assert config.cleanup_interval_seconds == pytest.approx(10.0)  # max(10, min(60, 0.25/3))
    ...
    def test_high_p99_timeout_config(self):
        ...
        assert config.heartbeat_timeout_seconds == pytest.approx(10.0)
    ...
    def test_cleanup_interval_calculation(self):
        ...
        assert config_small.cleanup_interval_seconds == pytest.approx(10.0)
        ...
        assert config_medium.cleanup_interval_seconds == pytest.approx(expected_interval)
        ...
        assert config_large.cleanup_interval_seconds == pytest.approx(60.0)
...
class TestRetryPolicy:
    ...
    def test_get_delay_exponential_backoff(self):
        ...
        assert policy.get_delay(0) == pytest.approx(BASE_DELAY)  # BASE_DELAY * 2^0
        assert policy.get_delay(1) == pytest.approx(MAX_DELAY)  # BASE_DELAY * 2^1
        assert policy.get_delay(2) == pytest.approx(0.4)  # BASE_DELAY * 2^2
        assert policy.get_delay(3) == pytest.approx(0.8)  # BASE_DELAY * 2^3
        assert policy.get_delay(4) == pytest.approx(1.6)  # BASE_DELAY * 2^4
        assert policy.get_delay(5) == pytest.approx(3.2)  # BASE_DELAY * 2^5
        assert policy.get_delay(6) == pytest.approx(5.0)  # Capped at max_delay
