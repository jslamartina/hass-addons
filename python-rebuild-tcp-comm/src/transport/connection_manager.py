...
class ConnectionManager:
    ...
    **Performance Monitoring**: Lock hold time is instrumented and monitored with three-tier thresholds:
    ...
    def __init__(
        ...
        self.framer = PacketFramer()  # One per connection
    ...
    async def _attempt_handshake(self, attempt: int, max_retries: int) -> bool:
        ...
        if not success:
            ...
            if self.pending_requests:
                self.pending_requests.popleft()  # Remove pending if send failed (FIFO)
            return False
        ...
        try:
            ...
            if self.pending_requests:
                self.pending_requests.popleft()  # Remove pending on timeout
            return False
        else:
            return True
    ...
    def _handle_data_packet(self, packet: CyncPacket) -> None:
        ...
        self._queue_packet_safe(packet)
        logger.debug(
            "✓ Data packet handled",
            extra={"packet_type": f"0x{packet.packet_type:02x}"},
        )
