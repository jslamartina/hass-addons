...
try:
    from cync_controller.transport.device_info import DEVICE_ID_LENGTH_BYTES, DEVICE_TYPE_LENGTH_BYTES
except ImportError:
...
# Generic JSON-compatible mappings used by the test helpers
JsonDict = dict[str, object]
PacketList = list[JsonDict]
...
def inject_packet(api_url: str, packet: bytes, direction: str = "CLOUD→DEV") -> JsonDict:
...
def parse_capture_logs(capture_file: Path, since_time: float | None = None) -> PacketList:
    ...
    with capture_path.open() as f:
        for raw_line in f:
            ...
            try:
                packet_obj = cast(object, json.loads(line))
                if not isinstance(packet_obj, dict):
                    continue
                packet_dict = cast(JsonDict, packet_obj)

                # Filter by timestamp if requested
                if since_time is not None:
                    timestamp_value = packet_dict.get("timestamp")
                    if isinstance(timestamp_value, str):
                        packet_time = datetime.fromisoformat(timestamp_value).timestamp()
...
def find_ack_for_msg_id(packets: PacketList, msg_id: int, ack_type: str = "7b") -> JsonDict | None:
    ...
    for packet in packets:
        hex_data = cast(str, packet.get("hex", ""))
        ...
        if len(hex_bytes) > MSG_ID_POSITION_CHECK and hex_bytes[MSG_ID_POSITION_CHECK] == msg_id_hex:
            return packet

    return None


def parse_mesh_info_response(hex_string: str) -> list[JsonDict]:
...
async def run_toggle_test(
    ...
    for i in range(iterations):
        ...
        try:
            ...
            if capture_file:
                ...
                if ack:
                    timestamp_val = ack.get("timestamp")
                    if isinstance(timestamp_val, str):
                        ack_time = datetime.fromisoformat(timestamp_val).timestamp()
                        ...
                        injection_result["ack"] = ack
                        print("  ✗ ACK missing timestamp")
                else:
                    print("  ✗ No ACK received")
            else:
                print("  ✓ Injected successfully")

            packets_list = cast(list[JsonDict], results["packets"])
    ...
    if latencies_list:
        ...
        results["latency_stats"] = {
            ...
            "p95": latencies[int(len(latencies) * 0.95)] if len(latencies) >= MIN_SAMPLES_FOR_P95 else latencies[-1],
            "p99": latencies[int(len(latencies) * 0.99)] if len(latencies) >= TIMEOUT_MS else latencies[-1],
        }

    return results


def _log_mesh_info_intro(endpoint: bytes, api_url: str, packet: bytes) -> None:
    """Print intro for mesh info test."""
...
def _mark_mesh_injection_success(results: JsonDict, response: object) -> None:
    """Record successful injection and log response."""
...
async def _collect_mesh_packets(capture_file: Path, start_time: float) -> list[JsonDict]:
    """Wait for mesh responses and parse capture logs."""
    print("Waiting for mesh info responses...")
    await asyncio.sleep(SLEEP_LONG_SECONDS)  # Give more time for large responses
    return parse_capture_logs(capture_file, since_time=start_time)


def _record_mesh_ack(packets: list[JsonDict], results: JsonDict) -> None:
    """Update results if ACK packet is found."""
    for packet_data in packets:
        hex_data = cast(str, packet_data.get("hex", ""))
...
def _record_mesh_devices(packets: list[JsonDict], results: JsonDict) -> int:
    """Parse mesh info responses and return count of devices added."""
    devices_added = 0
    for packet_data in packets:
        hex_data = cast(str, packet_data.get("hex", ""))

        # Mesh info data packets are 0x73 from DEV→CLOUD with large payloads
        if packet_data.get("direction") != "DEV→CLOUD" or not hex_data.startswith("73"):
            continue

        length_val = packet_data.get("length", 0)
        if isinstance(length_val, (int, float)) and length_val > MIN_MESH_INFO_PACKET_LENGTH:
            devices = parse_mesh_info_response(hex_data)
            if devices:
                devices_list = cast(list[JsonDict], results["devices"])
...
def _update_mesh_results_from_packets(packets: list[JsonDict], results: JsonDict) -> None:
    """Process captured packets for ACK and device data."""
    _record_mesh_ack(packets, results)
    devices_added = _record_mesh_devices(packets, results)
    if devices_added > 0:
...
async def run_mesh_info_test(
    ...
    _log_mesh_info_intro(endpoint, api_url, packet)
    ...
    try:
        response = inject_packet(api_url, packet)
        _mark_mesh_injection_success(results, response)
        if capture_file:
            packets = await _collect_mesh_packets(capture_file, start_time)
            _update_mesh_results_from_packets(packets, results)
    except Exception as e:
        print(f"✗ Injection failed: {e}")
...
def main():
    ...
    # Parse endpoint
    endpoint_arg = cast(str, args.endpoint)
    endpoint = bytes.fromhex(endpoint_arg.replace(" ", ""))
    ...
    capture_file_arg = cast(str | None, args.capture_file)
    capture_file = Path(capture_file_arg) if capture_file_arg else None

    # Run test
    api_url = cast(str, args.api_url)
    device_id = cast(int, args.device_id)
    iterations = cast(int, args.iterations)
    test_type = cast(str, args.test)

    if test_type == "toggle":
        results = asyncio.run(run_toggle_test(api_url, endpoint, device_id, iterations, capture_file))
    else:
        results = asyncio.run(run_mesh_info_test(api_url, endpoint, capture_file))
    ...
    if output_arg:
        output_path = Path(str(output_arg))
