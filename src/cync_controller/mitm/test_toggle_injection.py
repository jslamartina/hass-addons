        ...
        for raw_line in f:
            ...
            try:
                ...
                if since_time is not None:
                    timestamp_value = packet_obj.get("timestamp")
    ...
    try:
        ...
        if capture_file:
            ...
            for packet_data in packets:
                ...
                if (
                    packet_data.get("direction") == "DEV→CLOUD"
                    and hex_data.startswith("73")
                    and cast(int, packet_data.get("length", 0)) > MIN_MESH_INFO_PACKET_LENGTH
                ):
...
def main():
    ...
    # Run test
    if args.test == "toggle":
        results = asyncio.run(run_toggle_test(args.api_url, endpoint, args.device_id, args.iterations, capture_file))
    else:
