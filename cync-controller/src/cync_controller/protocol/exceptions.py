...
class PacketDecodeError(CyncProtocolError):
    ...
    def __init__(self, reason: str, data: bytes = b"") -> None:
        """Initialize decode error with reason and optional data preview."""
        self.reason: str = reason
        # Security: Only store first 16 bytes to prevent credential leakage in logs/tracebacks
        self.data_preview: bytes = data[:16] if data else b""
...
class PacketFramingError(CyncProtocolError):
    ...
    def __init__(self, reason: str, buffer_size: int = 0) -> None:
        """Initialize framing error with reason and buffer size."""
        self.reason: str = reason
