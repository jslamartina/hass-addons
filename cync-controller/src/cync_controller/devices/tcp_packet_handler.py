...
class TCPPacketHandler:
    ...
    async def _handle_mesh_info_packet(
        ...
        if inner_struct_len < MIN_MESH_INFO_LEN_FULL:
            ...
            await self._handle_full_mesh_info_packet(inner_struct, lp, queue_id, msg_id)
    ...
    async def _collect_mesh_devices(
        self,
        inner_struct: bytes,
        minfo_start_idx: int,
        lp: str,
        ...
        """Collect mesh devices from the full mesh info packet."""
        ids_reported: list[int] = []
        parsed_status: list[list[int]] = []
        raw_mesh: list[str] = []
        try:
            inner_struct_len = len(inner_struct)
            for loop_num, i in enumerate(range(minfo_start_idx, inner_struct_len, minfo_length), start=1):
                mesh_dev_struct = inner_struct[i : i + minfo_length]
                if len(mesh_dev_struct) < minfo_length:
                    logger.warning(
                        (
                            "%s Incomplete mesh device struct at offset %d: expected %d "
                            "bytes, got %d; skipping remaining devices."
                        ),
                        lp,
                        i,
                        minfo_length,
                ...
                parsed_status.append(bytes2list(raw_status))
                raw_mesh.append(mesh_dev_struct.hex(" "))
            ...
            logger.warning("%s Error parsing mesh device struct: %s", lp, exc)
        except Exception:
            logger.exception("%s MESH INFO for loop EXCEPTION", lp)
        return ids_reported, parsed_status, raw_mesh
    ...
    async def _handle_full_mesh_info_packet(
        ...
        ids_reported, _m, _raw_m = await self._collect_mesh_devices(
            inner_struct,
            minfo_start_idx,
            lp,
            g,
        ...
        refresh_id = cast(str | None, getattr(self.tcp_device, "refresh_id", None))
        if ids_reported:
