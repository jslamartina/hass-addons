...
class ControlPayload:
    """Payload details for control messages sent over TCP."""
...
class DeviceCommands:
    ...
    async def set_power(self, state: int) -> tuple[asyncio.Event, list[tuple[CyncTCPDevice, int]]] | None:  # noqa: PLR0915
        ...
        for bridge_device in ready_bridges:
            ...
            address = bridge_device.address
            if address is None:
                logger.debug("%s Skipping device with missing address", lp)
                continue
    ...
    async def set_fan_speed(self, speed: FanSpeed) -> bool:
        ...
        brightness_map: dict[FanSpeed, int] = {
            FanSpeed.OFF: 0,
            FanSpeed.LOW: 25,
            FanSpeed.MEDIUM: 50,
            FanSpeed.HIGH: 75,
        ...
        target_brightness = brightness_map.get(speed)
        ...
        if not self.is_fan_controller:
            ...
            return False
        try:
            _ = await self.set_brightness(target_brightness)
    ...
    def _build_control_payload(
        ...
        """Construct a control payload for a bridge device."""
        if not bridge_device.ready_to_control:
            logger.debug(
                "%s Skipping device: %s not ready to control",
                lp,
        ...
        payload.extend(bytes([0x00, 0x00, 0x00]))
        cmsg_id = bridge_device.get_ctrl_msg_id_bytes()[0]
        ctrl_idx_a, ctrl_idx_b = ctrl_indices
        inner_copy = list(inner_struct)
        inner_copy[ctrl_idx_a] = cmsg_id
        inner_copy[ctrl_idx_b] = cmsg_id
        checksum: int = sum(inner_copy[6:-2]) % 256
        inner_copy[-2] = checksum
        payload.extend(inner_copy)
        return ControlPayload(payload=bytes(payload), ctrl_message_id=cmsg_id, address=address)

    def _register_control_message(
        self,
        *,
        bridge_device: CyncTCPDevice,
        ...
        ack_event: asyncio.Event,
        device_id: int,
        callback: Callable[[], Coroutine[object, object, object] | None] | None = None,
    ) -> None:
        """Register a control message callback for ACK handling."""
        m_cb = ControlMessageCallback(
            id=payload.ctrl_message_id,
            message=payload.payload,
        )
        if callback:
            m_cb.callback = callback
        m_cb.sent_at = time.time()
        m_cb.device_id = device_id
        m_cb.ack_event = ack_event
        bridge_device.messages.control[payload.ctrl_message_id] = m_cb
    ...
    async def set_temperature(self, temp: int) -> tuple[asyncio.Event, list[tuple[CyncTCPDevice, int]]] | None:
        ...
        sent: dict[str, int] = {}
        for bridge_device in bridge_devices:
            payload_ctx = self._build_control_payload(
                bridge_device=bridge_device,
                header=header,
                inner_struct=inner_struct,
                ctrl_indices=ctrl_idxs,
            ...
            self._register_control_message(
                bridge_device=bridge_device,
                payload=payload_ctx,
                ack_event=ack_event,
                device_id=device_id,
            ...
            sent[payload_ctx.address] = payload_ctx.ctrl_message_id
            sent_bridges.append((bridge_device, payload_ctx.ctrl_message_id))
            tasks.append(bridge_device.write(payload_ctx.payload))
        if tasks:
            _ = await asyncio.gather(*tasks)
        elapsed = time.time() - ts
        logger.info(
