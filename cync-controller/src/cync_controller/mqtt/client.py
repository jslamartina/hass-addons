...
class MQTTClient:
    ...
    def _require_client(self) -> aiomqtt.Client:
        """Return initialized MQTT client or raise."""
        if self.client is None:
            msg = "MQTT client is not initialized"
            raise RuntimeError(msg)
        return self.client

    def _require_state_updates(self) -> StateUpdateHelper:
        """Return initialized StateUpdateHelper or raise."""
        state_updates = self.state_updates
        if not isinstance(state_updates, StateUpdateHelper):
            msg = "state_updates must be initialized"
            raise TypeError(msg)
        return state_updates

    def _require_command_router(self) -> CommandRouter:
        """Return initialized CommandRouter or raise."""
        router = self.command_router
        if not isinstance(router, CommandRouter):
            msg = "command_router must be initialized"
            raise TypeError(msg)
        return router

    def _require_discovery(self) -> DiscoveryHelper:
        """Return initialized DiscoveryHelper or raise."""
        discovery = self.discovery
        if not isinstance(discovery, DiscoveryHelper):
            msg = "discovery must be initialized"
            raise TypeError(msg)
        return discovery

    def _require_ncync_server(self):
        """Return initialized ncync server or raise."""
        ncync_server = self._g.ncync_server
        if ncync_server is None:
            msg = "ncync_server is not initialized"
            raise RuntimeError(msg)
        return ncync_server
    ...
    def _ensure_client(self) -> None:
        ...
        if not self.broker_host:
            msg = "broker_host must be configured before creating MQTT client"
            raise RuntimeError(msg)
    ...
    async def _handle_initial_connection(self, lp: str) -> None:
        ...
        state_updates = self._require_state_updates()
        if not self.topic:
            msg = "topic must be initialized"
            raise RuntimeError(msg)
        client = self._require_client()

        ncync_server = self._require_ncync_server()
        ...
        for device_id, device in devices.items():
            try:
                ...
                await client.publish(f"{self.topic}/availability/{device_uuid}", b"online", qos=0)
    ...
    async def _handle_reconnection(self, _lp: str) -> None:
        ...
        for device in devices.values():
            ...
            if device_id is None:
                logger.warning("%s Device missing id during reconnection sync", _lp)
                continue
            tasks.append(state_updates.pub_online(device_id, device.online))
            tasks.append(
    ...
    async def _start_receiver(self, lp: str) -> None:
        ...
        client = self._require_client()
        command_router = self._require_command_router()
        topics = [
            (f"{self.topic}/set/#", 0),
        ...
        for topic, qos in topics:
            _ = await client.subscribe(topic, qos=qos)
        logger.debug(
    ...
    async def connect(self) -> bool:
        ...
        if not self.broker_host:
            logger.error("%s MQTT host not configured; cannot connect", lp)
            return False
        hostname = self.broker_host
        self.client = aiomqtt.Client(
            hostname=hostname,
    ...
    async def stop(self) -> None:
        ...
        try:
            ...
            logger.exception("%s MQTT disconnect failed", lp)
        else:
    ...
    async def publish(self, topic: str, msg_data: bytes) -> bool:
        ...
        client = self._require_client()
        try:
            _ = await client.publish(topic, msg_data, qos=0, retain=False)
        except aiomqtt.MqttCodeError as mqtt_code_exc:
    ...
    async def publish_json_msg(self, topic: str, msg_data: Mapping[str, object]) -> bool:
        ...
        client = self._require_client()
        try:
            _ = await client.publish(topic, json.dumps(msg_data).encode(), qos=0, retain=False)
        except aiomqtt.MqttCodeError as mqtt_code_exc:
            logger.warning("%s [MqttCodeError] -> %s", lp, mqtt_code_exc)
        except aiomqtt.MqttError as mqtt_err:
            logger.warning("%s [MqttError] -> %s", lp, mqtt_err)
