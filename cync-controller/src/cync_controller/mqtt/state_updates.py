"""MQTT state update helpers for publishing device and group states.

Provides state publishing functionality for MQTT integration.
"""
...
def _get_g():
    """Get the global object instance from mqtt_client module.
    
    This function ensures we always get the current g instance,
    which is important for tests that patch g at runtime.
    """
    from cync_controller import mqtt_client
    return mqtt_client.g


class StateUpdater:
    """Helper class for MQTT state update operations."""
    ...
    async def update_device_state(self, device: CyncDevice, state: int) -> bool:
        ...
        # Call through client's send_device_status for backward compatibility with tests
        # This ensures mocks on client.send_device_status work correctly
        return await self.client.send_device_status(device, mqtt_dev_state)
