...
class TimeoutConfig:
    ...
    def __init__(self, measured_p99_ms: float = 51.0) -> None:
        ...
        """
        self.measured_p99_ms: float = measured_p99_ms

        # Calculate all timeouts from measured p99
        self.ack_timeout_seconds: float = (measured_p99_ms * 2.5) / 1000.0  # 2.5x p99
        self.handshake_timeout_seconds: float = self.ack_timeout_seconds * 2.5  # 2.5x ACK
        self.heartbeat_timeout_seconds: float = max(self.ack_timeout_seconds * 3, 10.0)  # max(3x ACK, 10s)
        # Cleanup interval: max(10, min(60, ack_timeout / 3)) seconds
        # This is the interval between cleanup runs, NOT a timeout value
        self.cleanup_interval_seconds: float = max(10.0, min(60.0, self.ack_timeout_seconds / 3))
        self.send_timeout_seconds: float = self.ack_timeout_seconds  # Match ACK timeout

    @override
    def __repr__(self) -> str:
        """Return string representation showing all calculated timeouts."""
        return (
...
class RetryPolicy:
    ...
    def get_delay(self, attempt: int) -> float:
        ...
        delay: float = base_delay * float(math.pow(2.0, attempt_int))

        # Cap at maximum delay
        delay = min(delay, self.max_delay_seconds)

        # Add jitter: random value between 0 and delay * jitter_factor (non-crypto use)
        jitter: float = random.uniform(0, delay * self.jitter_factor)  # noqa: S311
        return delay + jitter

    @override
    def __repr__(self) -> str:
        """Return string representation of retry policy."""
        return (
