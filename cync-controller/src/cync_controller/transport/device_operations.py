...
from cync_controller.protocol.packet_types import PACKET_TYPE_DATA_ACK
...
class DeviceOperations:
    ...
    # Timeout constants
    DEFAULT_MESH_INFO_TIMEOUT: float = 10.0  # Timeout for collecting mesh info responses
    DEFAULT_DEVICE_INFO_TIMEOUT: float = 5.0  # Default timeout for device info requests
    MESH_INFO_SEND_TIMEOUT: float = 5.0  # Timeout for sending mesh info request
    MAX_DEVICE_INFO_TIMEOUT: float = 60.0  # Maximum recommended timeout for device info requests

    # Device ID constants
    DEVICE_ID_LENGTH_BYTES: int = 4  # Standard device ID length in bytes

    # Cache constants
    MAX_CACHE_SIZE: int = 1000  # Maximum number of cached device info objects (LRU eviction)

    def __init__(self, transport: ReliableTransport, protocol: CyncProtocol) -> None:
        ...
        """
        self.transport: ReliableTransport = transport
        self.protocol: CyncProtocol = protocol
        self.is_primary: bool = False  # Primary device designation
        self.parse_mesh_status: bool = False  # Flag for parsing responses
        self.device_cache: OrderedDict[str, DeviceInfo] = OrderedDict()  # Parsed device structs (LRU)
        self._cache_lock: asyncio.Lock = asyncio.Lock()  # Protect cache operations
        self.logger_prefix: str = "[DeviceOps]"
    ...
    async def _parse_0x83_packet(self, packet: CyncPacket, correlation_id: str) -> list[DeviceInfo]:
        """Parse PACKET_TYPE_STATUS_BROADCAST (0x83) packet.

        Contains multiple DEVICE_TYPE_LENGTH_BYTES-byte device structs.
    ...
    async def _parse_device_struct(self, raw_bytes: bytes, correlation_id: str) -> DeviceInfo:
        """Parse DEVICE_TYPE_LENGTH_BYTES-byte device struct.

        Handles PACKET_TYPE_STATUS_BROADCAST (0x83) or PACKET_TYPE_DEVICE_INFO (0x43) packet payloads.
        ...
        state: dict[str, object] = {
            ...
            "brightness": int(state_raw[1] if len(state_raw) > 1 else 0),
            "raw": state_raw.hex(),
        }
