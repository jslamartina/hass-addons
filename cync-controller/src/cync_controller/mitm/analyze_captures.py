...
def analyze_jsonl_captures(jsonl_file: Path) -> dict[str, object]:
    ...
    with jsonl_file.open() as f:
        for line in f:
            if not line.strip():
                continue
            loaded = cast(dict[str, object] | list[object] | str | int | float | bool | None, json.loads(line))
    ...
    for pkt in packets:
        hex_value = pkt.get("hex")
        direction_value = pkt.get("direction")
        hex_data = hex_value if isinstance(hex_value, str) else ""
        packet_type = hex_data.split()[0] if hex_data else "unknown"
        direction = direction_value if isinstance(direction_value, str) else "unknown"
...
def main():
    """Run capture analysis entry point."""
    ...
    packets_list = cast("list[dict[str, object]]", analysis["packets"])
    for ptype in sorted(types_found):
        examples: list[dict[str, object]] = [
            p for p in packets_list if isinstance(p.get("hex"), str) and cast(str, p["hex"]).startswith(ptype)
        ]
        if examples:
            example = examples[0]
