...
def validate_fixtures(fixtures: dict[str, bytes], fixture_type: str) -> list[ValidationResult]:
...
def main() -> None:
    ...
    legacy_results: list[ValidationResult] = validate_fixtures(LEGACY_FIXTURES, "Legacy Test")
    all_results.extend(legacy_results)

    # Validate against real captured packets (if any)
    if REAL_FIXTURES:
        real_results: list[ValidationResult] = validate_fixtures(REAL_FIXTURES, "Real Captured")
    ...
    print("=" * 60)
    mismatches: list[ValidationResult] = [r for r in all_results if not r["match"]]
