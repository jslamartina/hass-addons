...
class Packet(TypedDict):
    """Parsed packet entry from capture file."""
...
def parse_capture_file(filepath: str) -> list[Packet]:
...
def filter_packets(packets: Iterable[Packet], packet_type: str) -> list[Packet]:
    """Filter packets by type."""
    packet_key = packet_type.lower().replace("0x", "")
    return [p for p in packets if p["packet_type"] == packet_key]


def show_statistics(packets: list[Packet]) -> None:
...
) -> dict[str, list[tuple[Packet, Packet, float]]]:
    """Extract request → ACK pairs."""
    pairs: defaultdict[str, list[tuple[Packet, Packet, float]]] = defaultdict(list)

    pending: dict[str, list[Packet]] = {
        "23": [],  # Handshake → 28
        "73": [],  # Data → 7b
        "83": [],  # Status → 88
        "d3": [],  # Heartbeat → d8
...
def show_ack_pairs(
    pairs: dict[str, list[tuple[Packet, Packet, float]]],
) -> None:
...
def main() -> None:
    ...
    _ = parser.add_argument("files", nargs="+", help="Capture files to analyze")
    _ = parser.add_argument("--filter", metavar="TYPE", help="Filter by packet type (e.g., 0x73)")
    _ = parser.add_argument("--stats", action="store_true", help="Show statistics")
    _ = parser.add_argument("--ack-pairs", action="store_true", help="Extract and show ACK pairs")
    _ = parser.add_argument("--limit", type=int, help="Limit output to N packets")

    args = parser.parse_args()
    files: list[str] = [str(path) for path in cast(list[str], args.files)]
    packet_filter: str | None = cast(str | None, args.filter)
    show_stats: bool = cast(bool, args.stats)
    should_show_ack_pairs: bool = cast(bool, args.ack_pairs)
    limit_arg: int | None = cast(int | None, args.limit)
    ...
    logger.info("Loaded %d packets from %d file(s)", len(all_packets), len(files))
    print()  # noqa: T201  # Blank line for formatting

    # Apply filter
    if packet_filter:
        all_packets = filter_packets(all_packets, packet_filter)
        logger.info("Filtered to %d packets of type %s", len(all_packets), packet_filter)
        print()  # noqa: T201  # Blank line for formatting
    ...
    # Show ACK pairs
    if should_show_ack_pairs:
        pairs = extract_ack_pairs(all_packets)
        show_ack_pairs(pairs)
        return

    # Default: show packets
    limit = limit_arg if limit_arg is not None else len(all_packets)
