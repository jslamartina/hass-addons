...
class TestCyncDeviceProperties:
    ...
    def test_brightness_validation(self):
        ...
        with pytest.raises(ValueError, match="Brightness must be between 0 and 255"):  # type: ignore[call-overload]
            device.brightness = -1
    ...
    def test_temperature_validation(self):
        ...
        # Invalid values should raise ValueError
        with pytest.raises(ValueError, match="Temperature must be between 0 and 255"):  # type: ignore[call-overload]
    ...
    def test_rgb_validation(self):
        ...
        # Invalid red
        with pytest.raises(ValueError, match="Red must be between 0 and 255"):  # type: ignore[call-overload]
            device.red = -1
        ...
        # Invalid green
        with pytest.raises(ValueError, match="Green must be between 0 and 255"):  # type: ignore[call-overload]
            device.green = -1
        ...
        # Invalid blue
        with pytest.raises(ValueError, match="Blue must be between 0 and 255"):  # type: ignore[call-overload]
            device.blue = -1
    ...
    def test_rgb_list_validation(self):
        ...
        # Invalid list length
        with pytest.raises(ValueError, match="RGB value must be a list of 3 integers"):  # type: ignore[call-overload]
            device.rgb = [255, 128]
    ...
    def test_online_validation(self):
        ...
        with pytest.raises(TypeError, match="Online status must be a boolean"):  # type: ignore[call-overload]
            device.online = "true"  # type: ignore[assignment]
    ...
    def test_bt_only_property(self):
        ...
        device2: CyncDevice = CyncDevice(cync_id=0x5678, wifi_mac="AA:BB:CC:DD:EE:FF")
...
class TestCyncDevicePropertyMethods:
    ...
    def test_is_dimmable_with_metadata(self):
        ...
        device: CyncDevice = CyncDevice(cync_id=0x1234, cync_type=7)  # Type 7 is a common light type

        # If metadata exists and is a LIGHT, check dimmable capability
        if device.metadata is not None and device.metadata.type == DeviceClassification.LIGHT:
            from cync_controller.metadata.model_info import LightCapabilities

            # Assert capabilities is LightCapabilities - fail test if misconfigured
            assert isinstance(device.metadata.capabilities, LightCapabilities), (
                f"Device type LIGHT must have LightCapabilities, got {type(device.metadata.capabilities)}"
            )
            dimmable = device.metadata.capabilities.dimmable  # type: ignore[union-attr]
    ...
    def test_supports_rgb_with_metadata(self):
        ...
        device._supports_rgb = None  # type: ignore[protected-access]
        ...
        if device.metadata is not None and device.metadata.type == DeviceClassification.LIGHT:
            ...
            color_support = device.metadata.capabilities.color  # type: ignore[union-attr]
    ...
    def test_supports_temperature_with_metadata(self):
        ...
        # Reset explicit value to test metadata fallback
        device._supports_temperature = None  # type: ignore[protected-access]
        ...
        if device.metadata is not None and device.metadata.type == DeviceClassification.LIGHT:
            ...
            tunable_white_support = device.metadata.capabilities.tunable_white  # type: ignore[union-attr]
...
class TestCyncDeviceCommandErrorPaths:
    ...
    async def test_set_fan_speed_invalid_speed(self, caplog: Any) -> None:  # type: ignore[assignment]
        ...
        with patch("cync_controller.devices.g") as mock_g:
            ...
            # Test with FanSpeed value
            from cync_controller.structs import FanSpeed

            # This should handle gracefully without raising an exception
            _ = await device.set_fan_speed(FanSpeed.OFF)  # Valid speed
