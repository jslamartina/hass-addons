...
class TestCyncTCPDevice:
    ...
    async def test_tcp_device_close_cancels_tasks(self) -> None:
        ...
        # Use mock factory for tasks - MagicMock needed for full mockability
        # (setting return_value on methods like done() and get_name())
        ...
        mock_task2.done.return_value = False  # type: ignore[reportAny]
        mock_task2.get_name.return_value = "task2"  # type: ignore[reportAny]
        ...
        tcp_device.tasks.send = mock_task2  # type: ignore[assignment]

        _ = await tcp_device.close()

        # Should have cancelled tasks
        assert mock_task1.cancel.called  # type: ignore[reportAny]
        assert mock_task2.cancel.called  # type: ignore[reportAny]
