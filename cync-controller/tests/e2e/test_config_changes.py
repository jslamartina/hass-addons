...
def test_tcp_whitelist_configuration():
    ...
    updated_config = get_addon_config(ADDON_SLUG)
...
def test_max_clients_configuration():
    ...
    # Get current config
    current_config = get_addon_config(ADDON_SLUG)
    original_max = current_config.get("tuning", {}).get("max_clients", 8)
...
def test_command_targets_configuration():
    ...
    # Get current config
    current_config = get_addon_config(ADDON_SLUG)
    original_targets = current_config.get("tuning", {}).get("command_targets", 2)
...
def test_expose_device_lights_toggle():
    ...
    # Get current config
    current_config = get_addon_config(ADDON_SLUG)
    original_expose = current_config.get("features", {}).get("expose_device_lights", True)
...
def test_invalid_config_rejected():
    ...
    result = update_addon_config(ADDON_SLUG, current_config)
