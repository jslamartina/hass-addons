...
tuning_config="$(bashio::config 'tuning')"
bashio::log.info "${LP} Add-on 'tuning' config: ${tuning_config}"
...
if bashio::services.available mqtt ; then
  ...
  bashio::log.info "${LP} HASS MQTT service available, pulled MQTT connection details from 'bashio::services mqtt'"
else
  bashio::log.error "${LP} HASS MQTT service not available using 'bashio::services.available mqtt'"
fi
