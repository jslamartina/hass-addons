...
def run_pyright() -> str:
    ...
    cmd = [str(basedpyright_exec), "--project", str(PROJECT_ROOT / "pyrightconfig.json")]
    return asyncio.run(_execute_command(cmd, PROJECT_ROOT))
...
def parse_pyright_output(output: str) -> dict[str, list[PyrightError]]:
    ...
    for line in output.splitlines():
        ...
        if file_match:
            ...
            if current_file and PROJECT_ROOT.as_posix() in current_file:
                current_file = current_file.replace(PROJECT_ROOT.as_posix() + "/", "")
            continue
